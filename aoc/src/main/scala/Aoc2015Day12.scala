import scala.annotation.tailrec
import scala.io.Source
import scala.util.Try
import scala.io.*

object Day12 extends App :

  sealed trait Json
  case class Obj(fields: Map[String, Json]) extends Json:
    def containsRed : Boolean = fields.view.values.count{
      case Str("red")   => true
      case _            => false
    } > 0
  case class NumValue(n: Int) extends Json
  case class Str(s: String) extends Json
  case class Arr(a: List[Json]) extends Json

  object Json:
    import P.*
    val string:   P[String] = for { _ <- char('"'); s <- satisfy(_.isLetter).zeroOrMore; _ <- char('"')} yield s.mkString("")
    val numValue: P[Json] = (char('-') ~ digits.map(x => NumValue(-x)) ) | digits.map(x => NumValue(x))
    val field:    P[(String,Json)] = for {s <- string; _ <- char(':') ;  n <- json} yield (s, n)
    val obj:      P[Json] = seq('{', ',', '}', field).map(x => Obj(x.toMap))
    val arr:      P[Json] = seq('[', ',', ']', json).map(x => Arr(x))
    val json:     P[Json] = arr | obj | numValue | string.map(s => Str(s))
    def parse(s: String) = json.run(s)

  import Json.*

  val input : Json =
    val s =
      Source
      .fromResource(s"inputAoc2015Day12.txt")
      .mkString
    parse(s)

  def solve1(json: Json) : Int =
    json match
      case NumValue(i)  => i
      case Str(i)       => 0
      case Arr(l)       => l.foldLeft(0)((a,j) => a+solve1(j))
      case Obj(o)       => o.view.values.foldLeft(0)((a,j) => a+solve1(j))

  def solve2(json: Json): Int =
    json match
      case NumValue(i) => i
      case Str(i)                     => 0
      case Arr(l)                     => l.foldLeft(0)((a, j) => a + solve2(j))
      case o@Obj(m) if !o.containsRed => m.view.values.foldLeft(0)((a, j) => a + solve2(j))
      case _                          => 0

  val start1 = System.currentTimeMillis
  val day: String = this.getClass.getName.drop(3).init
  val answer1 = solve1(input)
  println(s"Answer day ${day}, part 1: ${answer1} [${System.currentTimeMillis - start1}ms]")
  assert(answer1 == 111754)

  val start2 = System.currentTimeMillis
  val answer2 = solve2(input)
  println(s"Answer day ${day}, part 2: ${answer2} [${System.currentTimeMillis - start1}ms]")
  assert(answer2 == 65402)

/** Utilities */

object P:
  case class P[+A](parse: String => Option[(A, String)]):
    def run(s: String): A =
      parse(s) match
        case Some(a, "") => a
        case Some(_, rest) => sys.error(s"unconsumed at ${rest.take(10)}")
        case None => sys.error(s"failed to parse")

    def map[B](f: A => B): P[B] =
      P(s => parse(s) match
        case Some(a, r) => Some(f(a), r)
        case None => None
      )

    def flatMap[B](f: A => P[B]): P[B] =
      P(s => parse(s) match
        case Some(a, r) => f(a).parse(r)
        case None => None
      )

    private def loop(s: String, acc: List[A] = List.empty): (List[A], String) =
      parse(s) match {
        case None => (acc.reverse, s)
        case Some((a, ss)) => loop(ss, a :: acc)
      }

    def opt: P[Option[A]] =
      P(s => parse(s).map((a, ss) => (Some(a), ss)).orElse(Some(None, s)))

    def zeroOrMore: P[List[A]] =
      P(s => Some(loop(s)))

    def oneOrMore: P[List[A]] =
      P(s => parse(s).flatMap((a, ss) => Some(loop(ss, List(a)))))

    def |[A1 >: A](that: => P[A1]): P[A1] =
      P(s => parse(s) match {
        case None => that.parse(s)
        case res@Some(_) => res
      })

    def &[B](that: => P[B]): P[(A, B)] =
      for {a <- this; b <- that} yield (a, b)

    def ~[B](that: => P[B]): P[B] =
      for {_ <- this; b <- that} yield b

  def unit[A](a: A): P[A] =
    P(s => Some(a, s))

  def fail[A]: P[A] =
    P(_ => None)

  def take: P[Char] =
    P(s => if s.nonEmpty then Some(s.head, s.tail) else None)

  def satisfy(p: Char => Boolean): P[Char] =
    take.flatMap(c => if p(c) then unit(c) else fail)

  def char(c: Char): P[Char] =
    satisfy(_ == c)

  def digit: P[Char] =
    satisfy(_.isDigit)

  def digits: P[Int] =
    satisfy(_.isDigit).oneOrMore.map(_.mkString("").toInt)

  def separated[A](sep: P[_], pa: => P[A]): P[List[A]] =
    for {h <- pa.opt; t <- (sep ~ pa).zeroOrMore} yield
      h.map(_ :: t).getOrElse(List.empty)

  def separated[A](sep: Char, pa: => P[A]): P[List[A]] =
    separated(char(sep), pa)

  def seq[A](lhs: Char, sep: Char, rhs: Char, pa: => P[A]): P[List[A]] =
    for {_ <- char(lhs); es <- separated(sep, pa); _ <- char(rhs)} yield es
